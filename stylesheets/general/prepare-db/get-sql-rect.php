<?php
    ...
    foreach ( $sqls as $sql ) {
        ...
        echo "DROP TABLE IF EXISTS {$sql[1]}_$TILE;\n";    
        echo "CREATE TABLE {$sql[1]}_$TILE AS (SELECT * FROM (" . $sql[2]() . ") T WHERE ST_Intersects(way,ST_Transform(ST_SetSRID(ST_MakeBox2D(ST_GeomFromEWKT('SRID=4326;POINT($LON_START $LAT_START)'), ST_GeomFromEWKT('SRID=4326;POINT($LON_END $LAT_END)')), 4326),900913)));\n";
    }
