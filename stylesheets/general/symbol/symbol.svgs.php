...
foreach ( $RENDER_ZOOMS as $zoom ) {
	foreach ( $SYMBOL AS $selector => $a ) {
		if ( !empty($a['symbol-file']) && !empty($a['zooms']) && array_key_exists($zoom,$a['zooms']) ) {
			...
			$haloOpacity = empty($a['halo-opacity']) ? 0.33 : linear($a['halo-opacity'],$zoom);
			$haloRadius = empty($a['halo-radius']) ? 5 : linear($a['halo-radius'],$zoom);
