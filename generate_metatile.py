    ...
    for yfrac in range(0,fractions):
        ...
        for mapfile in mapfiles:
            ...
            if mapfile == 'hillshade':
                ...
                sys.stdout.flush()
                os.system("gdal_translate -q -projwin %(x1)f %(y2)f %(x2)f %(y1)f /home/klinger/mymap/data/tiles/aster/~%(tileX)s.%(tileY)s.hillshade.tif /tmp/~%(tileX)s.%(tileY)s.hillshade.tif" % {
                    'x1': bbox.minx,
                    'y1': bbox.miny,
                    'x2': bbox.maxx,
                    'y2': bbox.maxy,
